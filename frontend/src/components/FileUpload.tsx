...
const FileUpload: React.FC<FileUploadProps> = ({ onUploadSuccess }) => {
  ...
  // Carrega as categorias de vídeo
  useEffect(() => {
  ...
  const onDrop = (acceptedFiles: File[]) => {
    ...
    acceptedFiles.forEach(file => {
      ...
      if (['mp4', 'avi', 'mov', 'mkv', 'webm'].includes(extension || '')) {
        newFiles.video = file
        // Define automaticamente o nome do projeto com base no arquivo de vídeo, sem a extensão
        if (!projectName) {
  ...
  const handleUpload = async () => {
    if (!files.video) {
      message.error('Selecione um arquivo de vídeo')
      return
    }

    if (!files.srt) {
      message.error('Importe também o arquivo de legendas (.srt)')
      return
    }

    if (!projectName.trim()) {
      message.error('Digite o nome do projeto')
      return
    }
    ...
    try {
      ...
      message.success('Arquivos enviados com sucesso. Iniciando processamento...')
      
      // Redefine o estado
      setFiles({})
      setProjectName('')
      setUploadProgress(0)
      // Volta para a primeira categoria
      ...
      message.error('Falha no envio. Tente novamente.')
  ...
  return (
    <div style={{
      ...
      >
        ...
        <div>
          <Text strong style={{ 
            ...
            {isDragActive ? 'Solte para importar os arquivos' : 'Clique ou arraste os arquivos para esta área'}
          </Text>
          <Text style={{ color: '#cccccc', fontSize: '14px', lineHeight: '1.5' }}>
            Compatível com MP4, AVI, MOV, MKV e WebM. <Text style={{ color: '#ff9500', fontWeight: 600 }}>Também é obrigatório importar o arquivo de legendas (.srt)</Text>
          </Text>
        </div>
      </div>

      {/* Campo de nome do projeto — exibido após selecionar um arquivo */}
      {files.video && (
        <div style={{ marginBottom: '16px' }}>
          ...
          <Input
            ...
            placeholder="Digite o nome do projeto para identificar seu vídeo"
            style={{ 
              height: '40px',
              borderRadius: '12px',
              fontSize: '14px',
      ...
      {/* Seleção da categoria do vídeo — exibida após selecionar um arquivo */}
      {files.video && (
        <div style={{ marginBottom: '16px' }}>
          <Text strong style={{ color: '#ffffff', fontSize: '14px', marginBottom: '8px', display: 'block' }}>
            Categoria do vídeo
      ...
      {Object.keys(files).length > 0 && (
        <div style={{ marginBottom: '16px' }}>
          ...
          {/* Aviso de arquivo de legendas ausente */}
          {files.video && !files.srt && (
            <div style={{
              ...
              <Text style={{ color: '#ff9500', fontSize: '14px', fontWeight: 500 }}>
                Adicione um arquivo de legendas (.srt) para concluir a importação
              </Text>
            </div>
          )}
        </div>
      ...
      {uploading && (
        <div style={{ 
          ...
          <Text style={{ color: '#cccccc', fontSize: '13px', marginTop: '8px', display: 'block', textAlign: 'center' }}>
            Importando arquivos. Aguarde...
          </Text>
        </div>
      )}

      {/* Botão de upload — exibido após selecionar um arquivo */}
      {files.video && (
        <div style={{ textAlign: 'center', marginTop: '8px' }}>
          ...
          >
            {uploading ? 'Importando...' : 'Importar e processar'}
          </Button>
        </div>
      )}
    </div>
