...
const CollectionPreviewModal: React.FC<CollectionPreviewModalProps> = ({
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    try {
      ...
      hideLoading()
      message.error('Falha ao alterar a ordem dos clipes')
    } finally {
      setIsUpdating(false)
    }
  }
  ...
  const handleRemoveClip = async (clipId: string) => {
    ...
    const hideLoading = message.loading('Removendo clipe...', 0)
    setIsUpdating(true)
  ...
  const handleDownloadClip = async (clipId: string) => {
    ...
    try {
      ...
      message.success('Clipe baixado com sucesso')
    } catch (error) {
      console.error('Download clip failed:', error)
      message.error('Falha ao baixar o clipe')
    } finally {
  ...
  const handleDownloadCollection = async () => {
    ...
    try {
      ...
      message.error('Falha ao baixar a coleção')
    } finally {
  ...
  const handleGenerateVideo = async () => {
    ...
    try {
      ...
      message.success('A geração do vídeo da coleção foi iniciada. O download ficará disponível em seguida.')
    } catch (error) {
      message.error('Falha ao gerar o vídeo da coleção')
    } finally {
  ...
  const handleAddClips = async (selectedClipIds: string[]) => {
    ...
    const hideLoading = message.loading('Adicionando clipe...', 0)
    setIsUpdating(true)
    ...
    try {
      ...
      hideLoading()
      message.success(`${selectedClipIds.length} clipes adicionados à coleção com sucesso`)
    } catch (error) {
      console.error('Failed to add clips:', error)
      hideLoading()
      message.error('Falha ao adicionar o clipe')
    } finally {
      setIsUpdating(false)
    }
  }
                ...
                <div className="video-container">
                  {currentClip ? (
                    ...
                    <div className="empty-video">
                      ...
                      <Text style={{ color: '#999', marginTop: 16 }}>Nenhum conteúdo de vídeo disponível</Text>
                    </div>
                  )}
                </div>
              ...
              <div className="playlist-container">
                <div className="playlist-header">
                  <div>
                    ...
                    <Text type="secondary">Arraste para ajustar a ordem</Text>
                  </div>
                  {onAddClip && (
                    <Button 
                      type="primary" 
